...
class SEBasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # out = self.conv3(out)
        # out = self.bn3(out)
        # print("in sa: ", out.shape)
        # out = out * self.sa(out)
        # print("out sa: ", out.shape)

        if self.downsample is not None:
            # print("in downsample: ", x.shape)
            residual = self.downsample(x)
            # print("out downsample: ", residual.shape)
...
class DataEmbedding(nn.Module):
    ...
    def forward(self, x, x_mark, stage=2):
        if x_mark is None:
            if stage == 1:
...
class ARFEmbedding(nn.Module):
    ...
    def forward(self, x, x_mark=None, stage=2):
        # print("arf in", x.shape)
        x = self.ARF(x)
        # print("arf out", x.shape)
        x = self.flatten(x)
...
class CBAMEmbedding(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', dropout=0.1):
        super(CBAMEmbedding, self).__init__()
        self.inplanes = c_in
        self.ARF = self._make_layer(SEBasicBlock, d_model, 1)
        self.position_embedding = PositionalEmbedding(d_model=c_in)
        self.flatten = nn.Flatten()
    ...
    def _make_layer(self, block, planes, blocks, stride=2):  # makes residual SE block
        downsample = None
        if stride != 1 or self.inplanes != planes * block.expansion:
            # 通道下采样
            downsample = nn.Sequential(
