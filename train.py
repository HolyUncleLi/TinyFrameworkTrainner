    ...
    for epoch in range(model.get_current_epoch(), config["n_epochs"]):
        ...
        if best_acc < valid_outs["test/accuracy"] and \
           best_mf1 <= valid_outs["test/f1_score"]:
            ...
            best_test_acc = test_outs["test/accuracy"]
            best_test_mf1 = test_outs["test/f1_score"]
        ...
        #     break
    print("best acc: ", best_acc, "|best test acc: ", best_test_acc)
    print("best mf1: ", best_mf1, "|best test mf1: ", best_test_mf1)

    f = open("./output/acc.txt", "a")
    f.write(str(best_acc) + '\n')
    f.close()
    f = open("./output/mf1.txt", "a")
    f.write(str(best_mf1) + '\n')
    f.close()

    f = open("./output/best_test_acc.txt", "a")
    f.write(str(best_test_acc) + '\n')
    f.close()
    f = open("./output/best_test_mf1.txt", "a")
    f.write(str(best_test_mf1) + '\n')
    f.close()


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--config_file", type=str, default='sleepedf')
