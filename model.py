...
from models.TCN_Model import getmodel
...
class Model:
    ...
    def train_with_dataloader(self, minibatches):
        ...
        for x, y, w, sl, re in minibatches:
            ...
            y = y.to(self.device)
            w = w.to(self.device)
            y_pred = self.tsn.forward(x)
            # y_pred = torch.argmax(y_pred,dim=1)
            loss = self.CE_loss(y_pred, y.to(torch.long))
            # weight by sample
            ...
            '''
            cnn_weights = [parm for name, parm in self.tsn.model.cnn.named_parameters() if 'conv' in name]
            reg_loss = 0
